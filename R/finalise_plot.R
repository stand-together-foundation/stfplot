...
create_footer <- function (source_name, logo_image_path) {
  ...
  footer <- grid::grobTree(grid::linesGrob(x = grid::unit(c(0, 1), "npc"), y = grid::unit(1.1, "npc")),
                           grid::textGrob(source_name,
                                          x = 0.004, hjust = 0, gp = grid::gpar(fontsize=10, fontfamily="IBM Plex Serif")),
                           grid::rasterGrob(png::readPNG(logo_image_path), x = 0.9, width=unit(1,"in")))
  return(footer)

}

#' Arrange alignment and save STF ggplot chart
#'
#' Running this function will save your plot with the correct guidelines for publication for an STF graphic.
#' It will left align your title, subtitle and source, add the STF blocks at the bottom right and save it to your specified location.
...
#'  which needs to be a PNG file - defaults to STF logo that sits within the data folder of your package
...
finalise_plot <- function(plot_name,
                          ...
                          width_pixels=640,
                          height_pixels=450,
                          logo_image_path = file.path(system.file("data", package = 'stfplot'),"stf-logo.png")) {

  footer <- create_footer(source_name, logo_image_path)

  #Draw your left-aligned grid
  plot_left_aligned <- left_align(plot_name, c("subtitle", "title", "caption", "legend"))
